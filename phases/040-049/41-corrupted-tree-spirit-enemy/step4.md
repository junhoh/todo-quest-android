# Step 4: map-corrupted-tree-spirit-presentation
...
presentation 단위 테스트를 먼저 작성하고 구현 전 예상된 실패를 확인한 뒤 기존 종족 기반 visual catalog를 세 종족으로 확장한다. UI가 definition id, type 또는 grade를 직접 해석하지 않게 한다.
...
- `CORRUPTED_TREE_SPIRIT`은 `todo_quest_corrupted_tree_spirit_front_idle`, `battle_monster_corrupted_tree_spirit_name`, `battle_monster_corrupted_tree_spirit_death_announcement`을 사용한다.
- 기존 고블린과 해골 매핑은 변경하지 않는다.
- `BattlePresentationMapper.mapSnapshot`은 `CombatSnapshot.activeMonsterSpecies`를 catalog로 변환하는 기존 흐름을 유지한다.
- player 치명 공격 transition은 `before` 종족을 death scene에, `after` 종족을 spawn alert·spawn scene에 사용한다. 세 종족 사이 어느 방향으로 바뀌어도 이름·drawable·쓰러짐 안내가 섞이지 않아야 한다.
...
- Preview/sample에는 나무 정령 단일 사례를 추가하되 새로운 이미지나 animation frame을 만들지 않는다.

테스트는 catalog 매핑, BALANCED+NORMAL snapshot, Stage 1 encounter 1의 나무 정령, 나무 정령→해골·나무 정령→고블린·고블린/해골→나무 정령 치명 transition, nonlethal·monster attack 유지 사례를 포함한다.
...
5. task index의 step 4를 `completed`로 변경하고 세 종족 catalog와 transition 매핑 결정을 한국어 `summary` 한 줄로 기록한다.

## 금지사항

- `definitionId`, type 또는 grade 비교를 Compose나 ViewModel에 넣지 마라. 이유: 도메인 종족 정책과 presentation resource mapping을 분리해야 한다.
