# Step 3: register-corrupted-tree-spirit-runtime-resources
...
테스트를 먼저 작성해 runtime resource 부재로 실패하는 것을 확인한 뒤 canonical 나무 정령 PNG의 byte-identical 사본을 `/app/src/main/res/drawable-nodpi/todo_quest_corrupted_tree_spirit_front_idle.png`로 추가한다. 이미지를 다시 저장하거나 Android density 변환을 적용하지 않는다.

`/app/src/main/res/values/strings.xml`에 다음 한국어 문자열 리소스를 추가한다.

```xml
<string name="battle_monster_corrupted_tree_spirit_name">타락한 나무 정령</string>
<string name="battle_monster_corrupted_tree_spirit_death_announcement">타락한 나무 정령이 쓰러졌습니다.</string>
```

`scripts/test_validate_monster_sprite.py`에 canonical/runtime byte equality와 SHA-256 equality 테스트를 추가한다. 기존 고블린·해골 resource와 문자열은 유지한다. 새 bitmap 안에는 이름, 체력바, UI, 그림자, 이펙트와 배경을 합성하지 않는다.
...
$env:PYTHONUTF8='1'
.\.venv\Scripts\python.exe -m pytest scripts\test_validate_monster_sprite.py --basetemp build\pytest-41-tree-spirit-resource
...
- Compose 또는 ViewModel에 `타락한 나무 정령` 문장을 하드코딩하지 마라. 이유: AGENTS.md의 한국어 문자열 resource 규칙을 지켜야 한다.
- 기존 고블린·해골 resource를 덮어쓰지 마라. 이유: 세 종족은 독립 resource로 공존해야 한다.
- 기존 테스트를 깨뜨리지 마라.
