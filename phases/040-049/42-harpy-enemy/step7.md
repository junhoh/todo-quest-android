# Step 7: synchronize-harpy-docs-and-validate
...
- `docs/art/monster/README.md`에 네 번째 canonical PNG·spec·validator 명령과 byte-identical runtime 사본을 등록한다.
- `UI_GUIDE.md`에 하피의 `[11,11,53,58]`, 48px 높이, exact mirrored wing regions, 공통 외곽선·ground anchor·최근접 보간과 한국어 접근성 표시를 기록한다.
- `ARCHITECTURE.md`에 기존 `definitionId + grade → definition type → MonsterSpeciesPolicy → CombatSnapshot.activeMonsterSpecies → BattleMonsterVisualCatalog` 흐름과 네 종족 매핑을 기록한다.
- `ADR.md`의 Monster Combat/Battle Map 구현 상태에 `DEFENSE + NORMAL`을 하피로 파생하는 결정과 무 migration·무 balance change를 반영한다. 새 biome나 상성 ADR은 만들지 않는다.
- `docs/game-design/monster-stats-and-growth.md`에 정확한 type/grade 매핑 행렬, Stage 1 encounter 예시와 능력치·보상 불변을 기록한다.
- 필요한 경우 PRD와 docs index의 canonical 에셋 목록을 실제 구현과 일치하도록 최소 수정한다.
- `wind`, `flight`, `mountain`, `grassland`는 시각 콘셉트와 종족 식별 metadata이며 비행 동작, biome, 상성, 상태 효과, 스킬, 전리품이나 새 맵 배경을 추가하지 않는다고 명시한다.

전체 검증이 성공하면 task index의 step 7과 phase를 `completed`로 갱신하고 `/phases/index.json`의 `42-harpy-enemy`도 `completed`로 갱신한다. summary는 canonical/runtime PNG, 종족 정책, presentation과 실제 검증 결과를 한국어 한 줄로 기록한다.
...
$env:PYTHONUTF8='1'
.\.venv\Scripts\python.exe -m pytest scripts\test_validate_monster_sprite.py scripts\test_validate_character_sheet.py scripts\test_validate_battle_map.py scripts\test_execute.py --basetemp build\pytest-42-harpy-final
.\gradlew.bat test
.\gradlew.bat lint
...
1. 모든 AC 명령을 실행한다.
2. 네 canonical monster PNG, 하피 runtime byte equality, 1배율·8배율 하피와 portrait·landscape Battle Map을 확인한다.
3. 테스트 결과와 실제 코드·경로·Room version·종족 mapping을 문서와 대조한다.
4. ARCHITECTURE, ADR, UI_GUIDE, AGENTS.md의 CRITICAL 규칙을 최종 확인한다.
5. AC 성공 시 phase와 상위 index를 `completed`로 갱신한다. 필수 도구·기기 부재 시 설치하지 않고 `blocked`, 3회 수정 후에도 실패가 남으면 `error`로 기록한다.

## 금지사항

- 실제 구현과 다른 완료 수치나 검증 결과를 문서에 추정해 쓰지 마라. 이유: canonical 문서는 실행 증거와 일치해야 한다.
- Room migration, AGILE type, biome, 새 배경, 상성, 상태 효과, 스킬 또는 전리품을 추가하지 마라. 이유: 승인된 종족 표시 범위를 넘어선다.
